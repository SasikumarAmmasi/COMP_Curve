...
RATED_POWER = 4481
TEMP_DIR = 'temp_plots'
# In Streamlit, we generally don't use the file system for temp plots,
# but we'll keep the structure for clarity if needed.
# However, the plots will be generated in memory (BytesIO) for the Excel output.
# The os.makedirs and os.rmdir will be removed/modified in the main function.


# ----------------------------------------------------------------------
# PLOTTING FUNCTIONS (MODIFIED FOR STREAMLIT/IN-MEMORY FILE SAVING)
# ----------------------------------------------------------------------
...
def plot_superimposed_map_triple_axis(df, df_sorted, rated_power, pressure_value):
    ...
    # Rename the column used for the axis label (Temporarily handle the rename/check)
    # The original rename happened in the main function on the overall df.
    # We must ensure 'Actual Gas Flow (Am3/hr)' is present here.
...
def execute_plotting_and_excel_embedding():
    ...
    uploaded_file = st.file_uploader(
        "1. Upload your Excel data file (.xlsx)", 
        type=['xlsx'],
    ...
    st.subheader("2. Analysis & Plot Generation")
